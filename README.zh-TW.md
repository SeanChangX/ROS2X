...
它提供單一入口腳本，可處理映像建置、工作區建置、啟動/執行、micro-ROS 初始化、Foxglove Bridge 與 Groot2。
...
- Groot2 AppImage 啟動器（`./ROS2X app groot`）
- Foxglove Bridge 啟動器（`./ROS2X bridge`）
...
5. `app groot` 與 `bridge` 只有在本次呼叫是由它們啟動容器時才會停止容器。
   已存在且執行中的容器會保留。
